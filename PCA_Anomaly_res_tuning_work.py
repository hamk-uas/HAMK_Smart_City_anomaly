...
for n_components_index, n_components in enumerate(n_components_list):
    ...
    scaled_residual = pd.DataFrame(data = scaler.transform(x) - pca.inverse_transform(principalComponents), index = df_tempclean_train.index, columns = df_tempclean_train.columns)
